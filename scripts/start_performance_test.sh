...
check_ssh(){
    ...
    echo "ip=$ip"
    while true; do
        ssh -i /root/docker.key $net_node "ip netns exec $ns /usr/bin/sshpass -p 'cubswin:)' ssh -o StrictHostKeyChecking=no cirros@$ip exit"
        [ $? -eq 0 ] && break
        sleep 1 
        if [ $net_node == 'neutron1' ]; then
            net_node='neutron2'
        else
            net_node='neutron1'
        fi
...
do
    ...
    neutron router-gateway-set "$PREFIX-router$i" "$PREFIX-ext"
    ...
    nova boot --image cirros --flavor 1 --nic net-id=$net_id,v4-fixed-ip=$FIXIP "$PREFIX-instance$i"

    # 2.6 Associate floating ip to instance
    ip_addr=$(nova floating-ip-create $PREFIX-ext | grep -v -e "------" | grep -v -e "IP" | awk '{print $4}')
    ...
    check_ssh "$server_start_date" $net_node "qdhcp-$net_id" "$FIXIP"
    check_ssh "$floatingip_create_date" $net_node "qrouter-$router_id" $ip_addr
done
